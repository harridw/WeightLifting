...
There is a total of 160 variables in the data sets.  To avoid overfitting, we want to identify important or relevant variables, or features, that contribute significantly to greater predictive capability of a model.   
...
set.seed(100)
fitCtrl <- trainControl(method = "cv", number = 5,repeats = 1, classProbs = TRUE)
fitRPart <- train(classe ~ ., data = training[ , c("classe", keep)], method = "rpart", 
                  tuneLength = 20, trControl = fitCtrl, metric = "ROC")
predRPart <- predict(fitRPart, newdata = testing[ ,c("classe", keep)])
confusionMatrix(predRPart, testing$classe)

set.seed(100)
fitCtrl <- trainControl(method = "cv", number = 5,repeats = 1, classProbs = TRUE)
fitLDA <- train(classe ~ ., data = training[ , c("classe", keep)], method = "lda", 
                  tuneLength = 20, trControl = fitCtrl, metric = "ROC")
predLDA <- predict(fitLDA, newdata = testing[ ,c("classe", keep)])
confusionMatrix(predLDA, testing$classe)

set.seed(100)
fitCtrl <- trainControl(method = "cv", number = 5,repeats = 1, classProbs = TRUE)
fitRF <- train(classe ~ ., data = training[ , c("classe", keep)], method = "lda", 
                  tuneLength = 20, trControl = fitCtrl, metric = "ROC")
predRF <- predict(fitRF, newdata = testing[ ,c("classe", keep)])
