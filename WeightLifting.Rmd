...
packages <- c("plyr", "dplyr", "data.table", "dtplyr", "lubridate", "ggplot2", "scales",
                  ...
                  "verification", "pROC", "tree", "stringr")
ipak(packages)
```
...
validation <- read.csv(test.file, header = TRUE, 
                  na.strings = c("NA","N/A","NULL","Null","null",""," ","  ","#DIV/0!"))
```
...
```
```{r nzv_exclude, include = FALSE, echo = FALSE, results = "hide")}
...
predRPart <- predict(fitRPart, newdata = testing[ ,c("classe", keep)])
confuseRPart <- confusionMatrix(predRPart, testing$classe)

set.seed(100)
fitCtrl <- trainControl(method = "cv", number = 3,repeats = 1)
fitGBM1 <- train(classe ~ ., data = training[ , c("classe", keep)], method = "gbm", 
                  tuneLength = 10, trControl = fitCtrl, metric = "Accuracy")
predGBM1 <- predict(fitGBM1, newdata = testing[ ,c("classe", keep)])
confuseGBM1 <- confusionMatrix(predGBM1, testing$classe)

set.seed(100)
fitCtrl <- trainControl(method = "cv", number = 3,repeats = 1, 
                        classProbs = TRUE, summaryFunction = twoClassSuumary)
fitGBM2 <- train(classe ~ ., data = training[ , c("classe", keep)], method = "gbm", 
                  tuneLength = 10, trControl = fitCtrl, metric = "ROC")
predGBM2 <- predict(fitGBM2, newdata = testing[ ,c("classe", keep)])
confuseGBM2 <- confusionMatrix(predGBM2, testing$classe)

set.seed(100)
fitCtrl <- trainControl(method = "cv", number = 3,repeats = 1)
fitRF1 <- train(classe ~ ., data = training[ , c("classe", keep)], method = "rf", 
                  tuneLength = 10, trControl = fitCtrl, metric = "Accuracy")
predRF1 <- predict(fitRF1, newdata = testing[ ,c("classe", keep)])
confuseRF1 <- confusionMatrix(predRF1, testing$classe)
...
fitRF2 <- train(classe ~ ., data = training[ , c("classe", keep)], method = "rf", 
                  tuneLength = 10, trControl = fitCtrl, metric = "ROC")
predRF2 <- predict(fitRF2, newdata = testing[ ,c("classe", keep)])
confuseRF2 <- confusionMatrix(predRF2, testing$classe)
```
