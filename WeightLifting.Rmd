...
fitGBM <- train(classe ~ ., data = training[ , c("classe", keep)], method = "gbm", 
                  tuneLength = 10, trControl = fitCtrl, metric = "Accuracy")
predGBM <- predict(fitGBM, newdata = valid1[ ,c("classe", keep)])
confuseGBM <- confusionMatrix(predGBM, valid1$classe)

set.seed(100)
fitCtrl <- trainControl(method = "cv", number = 3,repeats = 1)
fitRF <- train(classe ~ ., data = training[ , c("classe", keep)], method = "rf", 
                  tuneLength = 10, trControl = fitCtrl, metric = "Accuracy")
predRF <- predict(fitRF, newdata = valid1[ ,c("classe", keep)])
confuseRF <- confusionMatrix(predRF, valid1$classe)
