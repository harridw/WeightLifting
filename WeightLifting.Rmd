...
feature.low <- unique(feature.low)   ## features with low importance for model -- exclude?
core.feature <- subset(training, select = -feature.low)
```

preProc <- preProcess(training, method = c("zv", "nzv", "corr"), thresh = 0.80, 
...
set.seed(100)
preProc <- preProcess(training, method = c("zv", "nzv", "corr"), 
                        na.remove = TRUE, use = "pairwise.complete.obs", 
                        freqCut = 80/20, uniqueCut = 1, cutoff = 0.8) 
fitCtrl <- trainControl(method = "cv", number = 5,repeats = 1,)
fitRPart <- train(classe ~ ., data = training, method = "rpart", 
                  tuneLength = 30, trControl = fitCtrl, metric = "ROC", 
                  preProc = preProc)
